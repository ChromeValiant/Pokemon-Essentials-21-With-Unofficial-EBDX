  ...
  DIALGA = {
    'turnStart0' =>
      proc do
        ...
        # show flavor text
        @scene.pbDisplay(
          'The ruler of time itself; Dialga starts to radiate tremendous amounts of energy!'
        )
        @scene.pbDisplay('Something is about to happen ...')
        # play common animation
        EliteBattle.playCommonAnimation(:ROAR, @scene, 1)
        @scene.pbDisplay(
          "Dialga's roar is pressurizing the air around you! You feel its intensity!"
        )
    ...
    'everyTurnStart' =>
      proc do
        ...
        if !battler.fainted?
          ...
          moveList =
            [
              %i[PSYCHIC RECOVER THUNDERBOLT HYPERBEAM],
              %i[PSYCHOBOOST ICEBEAM SHADOWBALL SUPERPOWER],
              %i[IRONWALL MIRRORCOAT COUNTER CALMMIND],
              %i[EXTREMESPEED PSYCHOBOOST ENERGYBALL PSYCHIC]
            ][
              newForm
            ]
          if newForm != oldForm
            ...
            moveList.each { |move| battler.pokemon.learn_move(move) }
